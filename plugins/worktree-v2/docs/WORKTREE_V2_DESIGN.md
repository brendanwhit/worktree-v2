# Worktree V2: Clean Room Orchestrator Design
...
│  ├─ sandbox_exists()    └─ ensure_local()                    │
│  └─ list_sandboxes()                                         │
│                                                              │
│  Each backend has:                                           │
│    - Real implementation (actual Docker/git/terminal)        │
...
│   │   └── registry.py     # Global worktree registry
│   ├── cli/
│   │   ├── ralph.py        # CLI entry point
│   │   ├── spawn.py
│   │   └── cleanup.py
│   └── tests/
│       ├── test_planner.py
...
```python
@pytest.mark.e2e
def test_ralph_spawns_sandbox():
    result = subprocess.run([
        "python", "cli/ralph.py",
        "--repo", TEST_REPO,
        "--task", "test task",
        "--wait"  # New flag: wait for agent to start
    ])
...
└── wt2-zzz.3: Global worktree registry

wt2-aaa: Epic: CLI & Commands
├── wt2-aaa.1: ralph.py CLI with all flags
├── wt2-aaa.2: spawn.py CLI (simpler, no Docker)
├── wt2-aaa.3: Slash command .md files
└── wt2-aaa.4: Resume and cleanup commands
...
1. Build v2 in separate repo (brendanwhit/worktree-v2)
2. Test thoroughly with Beads tracking
3. When stable, either:
   - Replace v1 in fdy-skills marketplace
   - Or publish as separate plugin, deprecate v1
