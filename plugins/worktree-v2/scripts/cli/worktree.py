"""Unified CLI for worktree management using typer.

Subcommands:
    run       — Create a worktree and spawn an agent
    list      — List all active worktree entries
    resume    — Resume an existing worktree
    cleanup   — Remove stale worktrees and registry entries
"""
...
def list_cmd() -> None:
    ...
    if not entries:
        typer.echo("No worktree entries found.")
    else:
        for entry in entries:
            sandbox_info = (
...
@app.command()
def resume(
    name: str = typer.Option(..., help="Name of the worktree entry to resume."),
) -> None:
    """Resume an existing worktree."""
    registry = get_default_registry()
    entry = resume_worktree(name, registry)
    if entry is None:
        typer.echo(f"Error: worktree '{name}' not found or path missing", err=True)
        raise typer.Exit(code=1)
...
def cleanup(
    name: str | None = typer.Option(
        None, help="Remove a specific worktree entry by name."
    ),
    ...
    """Remove stale worktrees and registry entries."""
    registry = get_default_registry()
