"""Tests for the unified worktree.py CLI (typer-based)."""
...
class TestRunCommand:
    ...
    def test_mode_and_target_as_positional_args(self) -> None:
        with (
            patch("cli.worktree.Planner") as mock_planner_cls,
            patch("cli.worktree.Executor") as mock_executor_cls,
        ):
...
class TestListCommand:
    ...
    def test_empty_registry(self, tmp_path: Path) -> None:
        ...
        with patch("cli.worktree.get_default_registry", return_value=registry):
            result = runner.invoke(app, ["list"])
            assert result.exit_code == 0
            assert "No worktree entries" in result.output
...
class TestCleanupCommand:
    ...
    def test_requires_name_or_all(self) -> None:
        with patch(
            "cli.worktree.get_default_registry",
...
class TestBusinessLogicFunctions:
    ...
    def test_list_worktrees_empty(self, tmp_path: Path) -> None:
        registry = WorktreeRegistry(tmp_path / "registry.json")
        assert list_worktrees(registry) == []

    def test_list_worktrees_populated(self, tmp_path: Path) -> None:
        registry = WorktreeRegistry(tmp_path / "registry.json")
        registry.add(
            WorktreeEntry(
                name="test",
    ...
    def test_resume_worktree_found(self, tmp_path: Path) -> None:
        registry = WorktreeRegistry(tmp_path / "registry.json")
        wt_path = tmp_path / "worktree"
        wt_path.mkdir()
        registry.add(
        ...
        entry = resume_worktree("test", registry)
        assert entry is not None
        assert entry.name == "test"

    def test_resume_worktree_not_found(self, tmp_path: Path) -> None:
        registry = WorktreeRegistry(tmp_path / "registry.json")
        assert resume_worktree("nonexistent", registry) is None

    def test_resume_worktree_missing_path(self, tmp_path: Path) -> None:
        registry = WorktreeRegistry(tmp_path / "registry.json")
        registry.add(
            WorktreeEntry(
                name="stale",
        ...
        assert resume_worktree("stale", registry) is None
