...
from superintendent.backends.docker import DockerBackend
...
class AuthBackend(Protocol):
    ...
    def setup_git_auth(self, sandbox_name: str) -> bool:
        """Configure git credential helper inside the sandbox."""
        ...

    def inject_token(self, sandbox_name: str, token: str) -> bool:
        """Inject a GitHub token and configure git auth in one step."""
        ...

    def validate_token(self, token: str, required_scopes: list[str]) -> bool:
        """Verify a token is valid and has the required OAuth scopes."""
        ...

    def setup_ssh_key(self, sandbox_name: str, key_path: Path) -> bool:
        """Copy an SSH key into the sandbox's ~/.ssh directory."""
        ...
